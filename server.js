...
const GITHUB_URL =
  ...
  "https://github.com/vikas-bhat-d/vtu-course-automation";
  ...
  runAutomation(
    ...
    (type, data) => {
      ...
      if (type === "lecture_done") {
        if (!data.retry) {
          if (data.status === "skip" && data.reason === "Already completed")
            job._alreadyDone = (job._alreadyDone || 0) + 1;
          const effective = data.completed + (job._alreadyDone || 0);
          job.processed = effective;
          job.progress = job.total
            ? Math.round((effective / job.total) * 100)
            : 0;
        }
      }
    ...
    .catch((err) => {
      ...
      job.result = {
        success: false,
        error: "Unexpected server error. Try again.",
      };
...
const submitLimit =
  process.env.NODE_ENV === "production"
    ? rateLimit({
        ...
        legacyHeaders: false,
        message: {
          error: "Too many requests. Even VTU has a rate limit (sort of).",
        },
      })
...
app.post("/api/submit", submitLimit, async (req, res) => {
  ...
  if (!/^[\w-]+$/.test(courseSlug)) {
    return res.status(400).json({
      error: "Invalid course slug. Use letters, numbers, and hyphens only.",
    });
  }
...
app.get("/api/status/:jobId", async (req, res) => {
  ...
  if (!job) {
    return res
      .status(404)
      .json({ error: "Job not found or has expired (jobs live for 1 hour)." });
  }
...
function adminAuth(req, res, next) {
  ...
  if (!adminPassword) {
    return res.status(503).json({
      error: "Admin access is not configured (ADMIN_PASSWORD not set in env).",
    });
  }
...
app.get("/api/admin/config", adminLimit, adminAuth, (req, res) => {
  ...
  for (const key of Object.keys(req.query)) {
    ...
    if (!(key in CONFIG_BOUNDS)) {
      return res.status(400).json({
        error: `Unknown config key: "${key}". Allowed: ${Object.keys(CONFIG_BOUNDS).join(", ")}.`,
      });
    }
    ...
    if (val < min || val > max) {
      return res
        .status(400)
        .json({ error: `"${key}" must be between ${min} and ${max}.` });
    }
    updates[key] = val;
  }
...
app.get("/api/admin/monitor", adminLimit, adminAuth, (_req, res) => {
  ...
  if (processing.length === 0 && queued.length === 0) {
    return res.json({
      message: "No active or queued jobs.",
      processing: [],
      queued: [],
    });
  }
...
app.get("/api/admin/notification", adminLimit, adminAuth, (req, res) => {
  ...
  if ("message" in req.query) {
    ...
    if (msg.length > 500) {
      return res.status(400).json({
        error: "Notification message must be 500 characters or less.",
      });
    }
    updates.message = msg;
  }
  ...
  if ("disabled" in req.query) {
    ...
    if (!["true", "false", "1", "0", "yes", "no"].includes(disabled)) {
      return res
        .status(400)
        .json({ error: `"disabled" must be true or false.` });
    }
...
async function restoreQueueFromRedis() {
  ...
  console.log(
    `[queue] Recovering ${queuedIds.length} job(s) from previous session...`,
  );
  const failedResult = {
    success: false,
  ...
  console.log(
    `[queue] ${queuedIds.length} job(s) marked as failed (no credentials after restart).`,
  );
}

app.listen(PORT, () => {
  console.log(`\n🛸  Course Pilot is live → http://localhost:${PORT}`);
  console.log(`    Course: "Please watch all 166 lectures."`);
  console.log(`    Us:  "No."\n`);
  ensureSeed().catch((e) => console.warn("[redis] seed failed:", e.message));
  restoreQueueFromRedis().catch((e) =>
    console.warn("[queue] restore failed:", e.message),
  );
});
