...
const JOB_TTL_SEC       = 3600;       // 1 hour — cleanup window after job finishes
const JOB_PENDING_TTL_SEC = 3600 * 24; // 24 hours — covers worst-case queue wait time
...
async function saveJob(jobId, jobData) {
  ...
  try {
    ...
    // Use long TTL for pending jobs — they may wait a long time in queue
    await redis.expire(`${JOB_PREFIX}${jobId}`, JOB_PENDING_TTL_SEC);
  } catch (e) {
...
async function updateJobFields(jobId, fields) {
  ...
  try {
    ...
    // Once terminal, shrink TTL to 1hr cleanup window; otherwise keep long TTL
    const isTerminal = fields.status === "done" || fields.status === "failed";
    await redis.expire(`${JOB_PREFIX}${jobId}`, isTerminal ? JOB_TTL_SEC : JOB_PENDING_TTL_SEC);
  } catch (e) {
...
async function enqueueJobId(jobId) {
  ...
  try {
    ...
    // Keep the queue list alive at least as long as the longest-lived pending job
    await redis.expire(QUEUE_KEY, JOB_PENDING_TTL_SEC);
  } catch (e) {
